...
async function main() {
    ...
    const hashedPassword = await auth_util_1.AuthUtil.hashPassword('Admin@123');
    ...
    const instructorPassword = await auth_util_1.AuthUtil.hashPassword('Instructor@123');
    ...
    const studentPassword = await auth_util_1.AuthUtil.hashPassword('Student@123');
    ...
    const students = await Promise.all(studentUsers.map((user) => prisma.student.upsert({
